 ...
 * 菜单目录：
 * 1.内容库主js；
 * 2.contentChoose.js  选择菜单的视图；
 * 3.contentList.js  列表数据的视图；
 * 4.contentDetail.js  弹窗数据的视图；
 */
...
function initTableResultView(table_result){
    ...
    for(var i=0;i<table_result.length;i++){
        ...
        tableView += ""
            ...
            +" <span>                                                                           "
            +" <select content_id='"+content_id+"' plan_in_use='"+plan_in_use+"' class='online_status' id='tr_"+content_id+"'>                                                                         "
            +statusSelectView
            +" </select>                                                                        "
            +" <span>                                                                           "
...
function addUpSatatusEventListiener(){
    ...
    $(".online_status").bind({
        click:function(){
            ...
            this_plan_id = $(this).attr("content_id");//当前的plan_id
        },
        change:function(){
...
function upStatusEventListener( this_plan_id,f_status){
    $.ajax({
        url:contextPath+"/action/content/contentManage/saveStatus.do",
        data:{
            "contentId":this_plan_id,
            "statusId":f_status
        },
        async:false,
        type:"POST",
